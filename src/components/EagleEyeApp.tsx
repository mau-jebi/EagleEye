...
// Import assignments from PDF - sample data
...
export function EagleEyeApp() {
  // Persistent state with localStorage
  const [classes, , classesLoaded] = useLocalStorage('eagleeye-classes', defaultClasses)
  const [assignments, setAssignments, assignmentsLoaded] = useLocalStorage('eagleeye-assignments', importedAssignments)
  ...
  }, [assignmentsLoaded, setAssignments])
  ...
  const filteredAssignments = useMemo(() => {
    ...
    return filtered
  }, [assignments, assignmentsLoaded, searchQuery, selectedClass, selectedStatus, selectedSmartFilter])

  // Add/Edit assignment
  const handleSaveAssignment = useCallback((assignmentData: Omit<Assignment, 'id' | 'created_at' | 'updated_at'>) => {
  ...
  }, [editingAssignment, setAssignments])
  ...
  const updateAssignmentStatus = useCallback((id: string, newStatus: Assignment['status']) => {
    setAssignments(prev => prev.map(a =>
      a.id === id ? {
        ...a,
        status: newStatus,
      ...
      <div className="max-w-7xl mx-auto flex gap-4 p-3 md:p-4">
        ...
        <aside className="hidden md:block w-64 bg-white rounded-lg p-4 border border-gray-200 h-fit">
          ...
          <div>
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded hover:bg-gray-100 flex items-center justify-between transition-colors ${
                      selectedSmartFilter === key ? `bg-${color.split('-')[1]}-100 hover:bg-${color.split('-')[1]}-100` : ''
                    }`}
                  >
            ...
            {selectedSmartFilter && (
              <button
                onClick={() => setSelectedSmartFilter(null)}
        ...
        `}>
          <div className="p-4 h-full overflow-y-auto">
            ...
            {/* Same content as desktop sidebar but with mobile interactions */}
            <div className="mb-6">
              <h2 className="font-semibold text-gray-900 mb-3 text-sm">Classes</h2>
              <div className="space-y-1">
                <button
                  onClick={() => {
                    setSelectedClass(null)
                  ...
                  className={`w-full text-left px-3 py-2 rounded hover:bg-gray-100 text-sm touch-manipulation transition-colors ${
                    !selectedClass && !selectedSmartFilter ? 'bg-gray-100' : ''
                  }`}
                >
                  All Classes
                ...
                {classes.map(cls => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 rounded hover:bg-gray-100 flex items-center gap-2 text-sm touch-manipulation transition-colors ${
                      selectedClass === cls.id ? 'bg-gray-100' : ''
                    }`}
                  >
                    <span
